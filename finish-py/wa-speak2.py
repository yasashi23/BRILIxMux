...
# LOOPING NYA
for ind,el in enumerate(datAnt):
    ...
    batStat = battery()
    txtBat = f"your battery now is {batStat}, is {batStat}, is {batStat}, normal battery!!"
    if(batStat > 33):
        txtBat = f"your battery now is {batStat}, is {batStat}, is {batStat}, normal battery!!"
    else:
        txtBat = f"your battery now is {batStat}, is {batStat}, is {batStat}, low battery low battery please charge!! please charge!!"

    tstn = f"Hello sir. there is a new post.  Job-Title:   {titleIsi[0]}. repeat once, Job-title: {titleIsi[0]}. Job-Skills:   {skill2}. repeat once, Job-Skills: {skill2}. Job-Type:   {titleIsi[6]}. repeat once, Job-type {titleIsi[6]}. thank you sir. {txtBat}"
    os.system(f"termux-tts-speak -r 0.5 {tstn}")
    ...
    ActionChains(driver).move_to_element(chat).click()\
        ...
        .key_down(Keys.SHIFT).send_keys(Keys.ENTER).key_up(Keys.SHIFT)\
        .send_keys(f"Battery Hp: *{batStat}%*")\
        .key_down(Keys.SHIFT).send_keys(Keys.ENTER).key_up(Keys.SHIFT)\
        .key_down(Keys.SHIFT).send_keys(Keys.ENTER).key_up(Keys.SHIFT)\
